...
import { LATEST_RELEASE_URL, RELEASES_URL } from './api-client/url';
...
describe('version-utils', () => {
	...
	describe('LATEST_KUROSEARCH_VERSION', () => {
		it('returns "Unavailable" when latest endpoint returns 404', async () => {
			const { LATEST_KUROSEARCH_VERSION } = await import('./version-utils');
			vi.stubGlobal(
				'fetch',
	...
	describe('ISLATESTVERSION', () => {
		it('returns true when latest matches newest release (v prefix normalized)', async () => {
			...
			const { ISLATESTVERSION } = await import('./version-utils');
